function CanvasDots(props) {
	
	const canvas2DotsBg = function () {
		const canvas2 = document.getElementById('canvas2Dots'),
		...
		// ctx.globalAlpha = 0.8;
		canvas2.width = document.body.scrollWidth;
		...
		let mousePosition = {
		  x: (30 * canvas2.width) / 100,
		  y: (30 * canvas2.height) / 100,
		};
		...
		Dot.prototype = {
		  ...
		  animate: function () {
			...
			for (let i = 1; i < dots.nb; i++) {
			  ...
			  if (dot.y < 0 || dot.y > canvas2.height) {
				dot.vx = dot.vx;
				dot.vy = -dot.vy;
			  } else if (dot.x < 0 || dot.x > canvas2.width) {
				dot.vx = -dot.vx;
				dot.vy = dot.vy;
			  }
			  dot.x += dot.vx;
	...
	return (
		<div>
			  <canvas id='canvas2Dots' class="canvas-dots"></canvas>
		</div>
	)
}
